...
const Profile: FC = () => {
  ...
  const [selectedBadgeDetails, setSelectedBadgeDetails] =
    useState<BadgeDetails | null>(null);
  ...
  const [
    getUserProfile,
    { data: myProfile, loading: loadingProfile, error: errorLoadingProfile },
  ] = useLazyQuery(GET_MY_PROFILE, { fetchPolicy: 'network-only' });

  const [
    getArchiveConcert,
    { data: concerts, loading: loadingConcerts, error: errorConcerts },
  ] = useLazyQuery(GET_ARCHIVE_CONCERT, { fetchPolicy: 'network-only' });

  const [
    updateProfile,
    { data: updateResponse, loading: loadingUpdate, error: updateError },
      ...
      if (formGroup && formGroup.classList.contains('focused')) {
        setTimeout(() => {
          ...
          const cursorPosition = Math.round(
            paddingLeft + borderLeft + textWidth
          );
              ...
              >
                {errorResponse?.error?.map((error) => (
                  <li key={error?.field} style={{ marginBottom: '4px' }}>
                    <strong>{error?.field?.toUpperCase()}:</strong>{' '}
                    {error.message}
                  </li>
                ))}
              </ul>
  ...
  const getBadgeDetails = (badgeType: string): BadgeDetails => {
    const relatedConcerts = archiveConcert.filter((concert) => {
      switch (badgeType) {
        case 'kuweni1':
          return ['Kuweni_1.1', 'Kuweni_1.2', 'Kuweni_1.3'].includes(
            concert.concert
          );
        case 'kuweni2':
          return [
    ...
    const badgeInfo = {
      kuweni1: {
        ...
        description:
          'In recognition of your participation during Kuweni Live in concert 2019/2020',
        image: 'images/badge/k1.png',
      },
      kuweni2: {
        title: 'Kuweni the Musical 2023',
        description:
          'In recognition of of your participation during Kuweni the musical 2023',
        image: 'images/badge/k2.png',
      },
      kuweniVerse: {
        title: 'Step into the Kuweniverse 2024',
        description:
          'In recognition of your contribution during Kuweniverse. Decoding and step into the Kuweniverse.',
        image: 'images/badge/k3.png',
      },
      yogeshwari: {
        title: 'Yogeshwari Treasure Hunt - Virtual',
        description:
          'Awarded for your key role in unraveling virtual puzzles and decoding the hidden path during the Yogeshwari Treasure Hunt.',
    ...
    return {
      ...
      description:
        badgeInfo[badgeType]?.description ||
        "You've participated in this concert series.",
          ...
          >
            ...
            <div className="badge-popup-body">
              ...
              <div className="badge-info">
                <h2 id="badge-title" className="badge-title">
                  {selectedBadgeDetails.title}
                </h2>
                <p id="badge-description" className="badge-description">
