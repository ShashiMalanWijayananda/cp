...
const PurchaseTickets: FC = () => {
  ...
  const handleBuyTicket = useCallback((couponCode, numberOfTicket, amount) => {
    ...
    ticketValidation({
      ...
      if (response?.code === "CODE-4000") {
        if (!response?.data?.validate) {
          appContext?.showErrorDialog("No enough ticket(s)", response?.data?.error)
        } else {
          if (amount > 0) {
            ...
            getPayhereHash({
              ...
              const hashResponse = res?.data?.generatePayHereHash as IAPIResponse;
              if (hashResponse?.code === "CODE-910") {
                const payment = {
                  ...
                  return_url: '',
                  cancel_url: '',
                  notify_url: 'https://backend-ticketing-v1-ewddbygwhxh5atgd.southeastasia-01.azurewebsites.net/ticketing/payments/ack',
              ...
              (window as any).payhere.onCompleted = function onCompleted(orderId: string) {
                ...
                setTimeout(() => {
                  validatPayhereRequest({
                    ...
                    const payHereRecordResponse = res?.data?.validatePayhereRequest as IAPIResponse;
                    if (payHereRecordResponse?.code === "CODE-912") {
                      const record: IPayHereRecord = payHereRecordResponse?.data;
                      if (record?.statusCode === "2") {
                        const tickets: ITicket[] = Array.from({length: numberOfTicket}, () => ({
                          requestId: user?.id,
                          zoneId: zone?.zoneId,
                          type: "general",
                          price: zone?.price,
                        ...
                        createTicketRequest({
                          ...
                          onCompleted: (data) => {
                            ...
                            if (response?.code === "CODE-014") {
                              appContext.showSuccessDialog("Success", data?.requestTicket?.message);
                            } else if (response?.code === "CODE-011") {
                              appContext.showSuccessDialog("Info", "Ticket purchasing success");
                              navigate("/my-tickets", {replace: true})
                            } else {
                              appContext.showSuccessDialog("Error", "Ticket processing error");
                            }
                          },
                          onError: (error) => {
                            console.error("Error creating tickets:", error);
                            appContext.showErrorDialog("Error", "Failed to create tickets. Please contact support.");
                          }
                        });
