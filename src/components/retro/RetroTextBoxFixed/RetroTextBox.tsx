        ...
        function updateCursorPosition(inputElement: HTMLInputElement) {
            ...
            if (formGroup && formGroup.classList.contains('focused')) {
                ...
                setTimeout(() => {
                    ...
                    // inside updateCursorPosition(...)
const computedStyle = window.getComputedStyle(inputElement);

// NEW: position relative to the wrapper, not just padding
const offsetLeft  = inputElement.offsetLeft; // relative to .input-wrapper
const paddingLeft = parseFloat(computedStyle.paddingLeft) || 8;
const borderLeft  = parseFloat(computedStyle.borderLeftWidth) || 0;

// NEW: include offsetLeft
let cursorPosition = Math.round(offsetLeft + paddingLeft + borderLeft + textWidth - 1);

const inputRect    = inputElement.getBoundingClientRect();
const paddingRight = parseFloat(computedStyle.paddingRight) || 8;

// NEW: max/min also include offsetLeft
const maxCursorPosition = offsetLeft + inputRect.width - paddingRight - borderLeft - 8;
const minCursorPosition = offsetLeft + paddingLeft + borderLeft;

if (cursorPosition > maxCursorPosition) {
  cursorPosition = Math.max(maxCursorPosition, minCursorPosition);
}
if (cursorPosition < minCursorPosition) {
  cursorPosition = minCursorPosition;
}

// Also pass font-size to CSS so the block matches the text height
formGroup.style.setProperty('--cursor-left', `${cursorPosition}px`);
formGroup.style.setProperty('--cursor-font-size', computedStyle.fontSize);
